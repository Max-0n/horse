...
class CarExampleScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 1; i < roadPoints.length; i++) {
      const a = roadPoints[i - 1] ?? { x: 0, y: 0 }, b = roadPoints[i] ?? { x: 0, y: 0 };
    ...
    // Wheels (physics only)
    this.leftWheel = this.matter.add.circle(carX-32, carY+22, 12, {
      density: 0.0012,
      friction: 3.5,
      restitution: 0.18,
      label: 'left-wheel',
    });
    this.rightWheel = this.matter.add.circle(carX+32, carY+22, 12, {
      density: 0.0012,
      friction: 3.5,
      restitution: 0.18,
      label: 'right-wheel',
    });
    // Constraints (axles, springs)
    this.matter.add.constraint(this.carBody, this.leftWheel, 24, 0.32, { pointA: { x: -32, y: 14 }});
    this.matter.add.constraint(this.carBody, this.rightWheel, 24, 0.32, { pointA: { x: 32, y: 14 }});
    ...
    // left wheel
    debugGraphics.strokeCircle(-32, 22, 12);
    // right wheel
    debugGraphics.strokeCircle(32, 22, 12);
